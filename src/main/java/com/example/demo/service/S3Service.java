...
public class S3Service {
    ...
    // S3 에 연결해서 인증을 처리 하는 로직
    @PostConstruct // S3Service 가 생성 될 때 1 번만 실행되는 아노테이션
    ...
    /**
     * 버킷에 파일을 업로드 하고 , 업로드한 버킷의 url 정보를 리턴
    ...
    public String uploadToS3Bucket(byte[] uploadFile, String fileName){
        ...
        // 업로드 할 파일을 S3 오브젝트 생성
        PutObjectRequest request = PutObjectRequest.builder()
                .bucket(bucketName) // 버킷 이름
                .key(fileName) // 파일명
                .build();

        // 오브젝트를 버킷에 업로드 (위에서 생성한 오브젝트, 업로드 하고자 하는 파일 (바이트 파일))
